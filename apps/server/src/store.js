...
export const createStore = (db) => {
  ...
  const listVolumes = () => db.prepare('select * from volumes order by orderIndex asc').all()

  const upsertVolume = (volume) => {
  ...
  const updateChapterContent = ({ id, content, wordCount, updatedAt, revision }) => {
  ...
  const createVersion = ({ id, chapterId, snapshot }) => {
    const timestamp = now()
  ...
  const listComments = (chapterId) =>
    db.prepare('select * from comments where chapterId = ? order by createdAt desc').all(chapterId)

  const addComment = ({ id, chapterId, author, body }) => {
    const timestamp = now()
